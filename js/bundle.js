...
function calc() {
  ...
  function calcTotal() {
    let res; // console.log(sex, params.height, params.weight, params.age, ratio);
    ...
    if (sex === "female") {
      res = (447.6 + 9.2 * params.weight + 3.1 * params.height - 4.3 * params.age) * ratio;
    } else {
      res = (88.36 + 13.4 * params.weight + 4.8 * params.height - 5.7 * params.age) * ratio;
    }
  ...
  function getStaticInfo(action, parent) {
    document.querySelector(`#${parent}`).addEventListener(action, e => {
      ...
      if (parent === "constitution") {
        params[elem.id] = checkNum(+document.querySelector(`#${elem.id}`).value, elem);
      }
  ...
  function makeElActive(el, actClass, parent) {
    document.querySelectorAll(`#${parent} div`).forEach(item => item.classList.remove(actClass));
...
function forms(formSelector, modalSelector, modalIntervalId) {
  ...
  function bindPostData(form) {
    form.addEventListener("submit", e => {
      ...
      const formData = new FormData(form); //put form values to object  - 1 variant
      // const obj = {};
      ...
      const json = JSON.stringify(Object.fromEntries(formData.entries())); //USE FETCH
      // postData("http://localhost:3000/requests", JSON.stringify(obj))

      (0,_services_services__WEBPACK_IMPORTED_MODULE_1__.postData)("http://localhost:3000/requests", json).then(req => {
        console.log(req);
...
function modal(triggerSelector, modalSelector, modalIntervalId) {
  ...
  const modalBtns = document.querySelectorAll(triggerSelector),
        modalWindow = document.querySelector(modalSelector); //1 var

  modalBtns.forEach(item => {
    item.addEventListener("click", () => openModal(modalSelector, modalIntervalId));
  });
...
function slider(_ref) {
  ...
  const slider = document.querySelector(container); //VER.1
...
function tabs(tabsSelector, tabsContentSelector, tabsParentSelector, activeClass) {
  ...
  const showTabContent = function () {
    let elem = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 0;
...
const getResource = async url => {
  const res = await fetch(url); //check for errors in fetch
...
/* harmony import */ var _modules_test__WEBPACK_IMPORTED_MODULE_8__ = __webpack_require__(/*! ./modules/test */ "./js/modules/test.js");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  //show modal after 3s
  const modalIntervalId = setTimeout(() => (0,_modules_modal__WEBPACK_IMPORTED_MODULE_2__.openModal)(".modal", modalIntervalId), 3000);
  (0,_modules_tabs__WEBPACK_IMPORTED_MODULE_1__["default"])(".tabheader__item", ".tabcontent", ".tabheader__items", "tabheader__item_active");
  (0,_modules_modal__WEBPACK_IMPORTED_MODULE_2__["default"])("[data-modal]", ".modal", modalIntervalId);
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_3__["default"])(".timer", "2022-09-22");
  (0,_modules_cards__WEBPACK_IMPORTED_MODULE_4__["default"])();
  (0,_modules_calc__WEBPACK_IMPORTED_MODULE_5__["default"])();
  (0,_modules_forms__WEBPACK_IMPORTED_MODULE_6__["default"])("form", ".modal", modalIntervalId);
